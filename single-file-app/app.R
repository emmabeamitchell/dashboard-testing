...
ui <- fluidPage(
  ...
  # Value Boxes shown only when dataset is five_year_outlook
  conditionalPanel(
    condition = "input.dataset_selector == 'five_year_outlook'",
    fluidRow(
      column(width = 6, div(class = "value-box-custom", valueBoxOutput("fiveyr_value_box_use"))),
      column(width = 6, div(class = "value-box-custom", valueBoxOutput("fiveyr_value_box_supply")))
    ),
    fluidRow(
      column(width = 6, div(class = "value-box-custom", valueBoxOutput("fiveyr_value_box_aug"))),
      column(width = 6, div(class = "value-box-custom", valueBoxOutput("fiveyr_value_box_red")))
    )
  )
)
...
server <- function(input, output, session) {
  ...
  # Filter and reshape 5-year data
